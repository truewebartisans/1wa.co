...
Как и всё в современном фронтенде, мы начинаем с установки React.js (с помощью `create-react-app`) и вспомогательных библиотек.
...
В рамках данной статьи, мы **не будем** использовать в визуальной части фронтенда ничего, кроме стандартной «Its works!» страницы React.js.

Для домашнего задания, можете поэкспериментировать, например, с библиотекой визуальных компонентов Material UI ([GitHub](https://github.com/mui-org/material-ui)). Если хотите полноценный пример web-приложения с использованием Material UI, то напишите об этом в комментариях!

#### Полный листинг ./ui/src/App.js
...
- Добавьте в функцию `showMessage` больше JSON данных для вывода на фронтенд. _Пример: добавьте новый атрибут `Emoji` (с любимым смайлом) в структуру `Message` и выведите его после атрибута `Text`_.
